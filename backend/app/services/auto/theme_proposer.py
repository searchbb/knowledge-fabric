        ...
        try:
            ...
            messages = [
                {"role": "system", "content": (
                    ...
                    "请生成一个**可跨多篇文章复用**的上位知识域主题名（不超过 15 字），"
                    "以及一句话描述（30-60 字）和 3-5 个关键词。\n"
                    "\n"
                    "命名原则（强约束）：\n"
                    "1. 主题名必须是**上位抽象概念**，可以承载多篇同类文章，"
                    "而不是只贴合当前这一篇。\n"
                    "2. 禁止使用当前文章才有的**具体领域名词**（例如：'政治'、'中国'、"
                    "'公司 X'、'产品 Y'、特定国家/行业/组织名）。要用可迁移的词根"
                    "（例如：用'个体心理与群体态度'而非'政治人格与体制支持'；"
                    "用'组织归属与成员行为'而非'某公司的员工忠诚'）。\n"
                    "3. 禁止使用文章标题、单个产品名、具体公司名。\n"
                    "4. 问一下：半年后如果来一篇相似主题但不同领域的文章，"
                    "这个主题名能不能自然地把它收进来？不能的话就太窄了。\n"
                    "\n"
                    "严格返回 JSON，不要输出 markdown。"
                )},
                {"role": "user", "content": (
